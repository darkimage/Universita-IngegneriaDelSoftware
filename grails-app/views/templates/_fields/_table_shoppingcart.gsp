                ...
                <g:each in="${columnProperties}" var="p" status="j">
                    <g:if test="${p.property=='quantity'}">
                        <td data-title="${message(code:headers + p.label.toLowerCase().replace(' ',''))} " class="align-middle">
                            <g:form controller="${params.controller}" action="update" method="PUT"
                            style="display:inline;margin-left:10px">
                                    <div class="input-group flex-nowrap">
                                        <input type='number' name='value' id='${"value" + bean['id']}' min='1' max="${bean['subProduct'].quantity}" value="${bean[p.property]}" class="form-control"/>
                                        <div class="input-group-append">
                                            <span class="input-group-text" id="basic-addon1">
                                                <input type='hidden' name='id' id='${"id" + bean['id']}' value="${bean['id']}"/>
                                                <g:formButtonImg dir="asset/images" image="update-icon.png" name="update" id="${"update" + bean['id']}" value="update"/>
                                                <g:formButtonImg dir="asset/images" image="delete-icon.png" name="delete" id="${"delete" + bean['id']}" value="delete"/>
                                            </span>
                                        </div>
                                    </div>
                            </g:form>
